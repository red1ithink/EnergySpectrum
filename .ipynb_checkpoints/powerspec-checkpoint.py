...
#########################
def spectra_flux(_u, _v, x, y, window=None, detrend=None):
    """
    2차원 난류에서 운동에너지(Kinetic Energy) 스펙트럼 플럭스를 계산하는 함수
    입력:
        _u, _v : 2D 속도장 (1024 x 1024)
        x, y   : 격자 좌표 (1024 x 1024)
        window : None, 'Hanning', 'Tukey' (창 함수 적용)
        detrend: 'Both', 'zonal', 'RemoveMean', 'RmeanDtrend', None (디트렌드 옵션)
    반환:
        wavenumber : 파수 벡터 (1D)
        flux       : 운동에너지 스펙트럼 플럭스
    """
    Nj, Ni = _u.shape  # 격자 크기

    # 1️⃣ 디트렌드 적용 (필요하면)
    if detrend:
        _u = _detrend_data(x, y, _u, detrend)
        _v = _detrend_data(x, y, _v, detrend)

    # 2️⃣ 윈도우 함수 적용 (필요하면)
    if window:
        _u = _apply_windowing(_u, window)
        _v = _apply_windowing(_v, window)

    # 3️⃣ 파수 계산
    wavenumber, wavenumber_2D, kx, ky, dx, dy = _wavenumber_vector(x, y, Ni, Nj)

    # 4️⃣ 속도 그래디언트 계산
    dudx, dudy, dvdx, dvdy = _get_grad_uv_in_fourier_space(_u, _v, kx, ky, Ni, Nj)

    # 5️⃣ 비선형 항 계산
    phi1 = _u * dudx + _v * dudy
    phi2 = _u * dvdx + _v * dvdy

    # 6️⃣ FFT 변환 후 에너지 전달항 계산
    tm1 = (fft.fft2(_u).conj()) * fft.fft2(phi1)
    tm2 = (fft.fft2(_v).conj()) * fft.fft2(phi2)
    
    transfer_2D = -1 * np.real(tm1 + tm2) / np.square(Ni * Nj)  # KE 전달항
    transfer_1D = _get_transfer1D(wavenumber_2D, wavenumber, transfer_2D)  # 등방성 가정하여 1D 변환

    # 7️⃣ 플럭스 계산 (누적합)
    flux = np.cumsum(transfer_1D[::-1])[::-1]
